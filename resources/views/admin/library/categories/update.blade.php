...
@section('main')
    <main class="h-full pb-16 overflow-y-auto">
        <div class="container px-6 mx-auto grid">
            ...
            <form action="{{ route('admin.library.categories.update', $categorybook->id) }}" method="POST"
                ...
                <div class="block text-sm">
                    ...
                    <input name="name" type="text"
                        ...
                        value="{{ old('name', $categorybook->name) }}" placeholder="Ingresa un nombre" />
                </div>
                <button type="submit"
                    class="mt-4 px-4 py-2 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-lg active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple">
                    Actualizar Categoría
                </button>
                <button action="{{ route('admin.library.categories.index') }}" type="submit" 
                    class="mt-4 px-4 py-2 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-lg active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple">
                    Cancelar
                </button>
