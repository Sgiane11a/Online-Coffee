    ...
    <ul class="flex gap-10 justify-center"> <!-- Aumenta el espacio entre los botones de las categorías -->
        @foreach ($categories as $category)
            <button wire:click="filterByCategory({{ $category->id }})"
                class="w-32 font-semibold {{ $selectedCategory == $category->id ? 'bg-raspberry-900 border-raspberry-500 border text-white p-3 rounded-xl' : 'bg-white border-white border text-black p-3 rounded-xl' }}">
                {{ $category->name }}
            </button>
        @endforeach
    </ul>
